...
import { ActiveView, Property, Broker, ContactMessage, Client, AboutUsConfig } from './types';
...
export default function App() {
  ...
  const [selectedPropertyId, setSelectedPropertyId] = useState<string>('casa-modernista-jardins');
  const [selectedBrokerId, setSelectedBrokerId] = useState<string>('ricardo-fontes');
  const language = 'pt';
  ...
  const [aboutUsConfig, setAboutUsConfig] = useState<AboutUsConfig>(DEFAULT_ABOUT_US);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const [fetchedProps, fetchedBrokers, fetchedClients, fetchedAboutUs] = await Promise.all([
          fetchPropertiesFromFirebase(),
          fetchBrokersFromFirebase(),
          fetchClientsFromFirebase(),
          fetchAboutUsFromFirebase()
        ...
        console.error("Erro ao carregar dados do Firebase, usando fallback local:", err);
      } finally {
  ...
  const handleMessageSubmit = (msgData: Omit<ContactMessage, 'id' | 'date'>) => {
  ...
  const filteredProperties = properties.filter((property) => {
    if (searchFilters.location !== 'Todos' && property.location !== searchFilters.location) {
      return false;
    }
    if (searchFilters.type !== 'Todos' && property.type !== searchFilters.type) {
      return false;
    }
    if (searchFilters.maxPrice > 0 && property.price > 0 && property.price > searchFilters.maxPrice) {
      return false;
    }
    return true;
  });

  const selectedProperty = properties.find((p) => p.id === selectedPropertyId) || properties[0] || PROPERTIES[0];
  const selectedBroker = brokers.find((b) => b.id === selectedBrokerId) || brokers[0] || BROKERS[0];
  
  // Custom Property detail broker
  const propertyBroker = brokers.find((b) => b.id === selectedProperty.brokerId) || brokers[0] || BROKERS[0];

  // Lookups for linked owner and buyer clients
  const propertyOwner = clients.find(c => c.type === 'owner' && (c.id === selectedProperty.ownerId || c.propertyId === selectedProperty.id));
  const propertyBuyers = clients.filter(c => c.type === 'buyer' && c.propertyId === selectedProperty.id);
        ...
        <AnimatePresence mode="wait">
          ...
          {currentView === 'home' && (
            ...
            >
                ...
                <div className="max-w-7xl mx-auto px-6">
                  <div className="bg-yellow-600/5 border border-yellow-600/10 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="text-sm">
                      {language === 'pt' ? 'Resultados da busca para:' : 'Search results for:'}{' '}
                      <span className="font-bold text-gray-900">
                        {searchFilters.location !== 'Todos' ? searchFilters.location : ''}
                        {searchFilters.location !== 'Todos' && searchFilters.type !== 'Todos' ? ' • ' : ''}
                        {searchFilters.type !== 'Todos' ? searchFilters.type : ''}
                        {searchFilters.maxPrice > 0 ? ` • ${language === 'pt' ? 'Até' : 'Up to'} ${formatBRL(searchFilters.maxPrice)}` : ''}
                      </span>
                      <span className="text-gray-500 ml-2">({filteredProperties.length} {language === 'pt' ? 'imóveis encontrados' : 'properties found'})</span>
                    </div>
                    <button
                      onClick={handleResetFilters}
                      className="text-xs font-bold text-yellow-600 hover:text-black hover:underline transition-all cursor-pointer"
                    >
                      {language === 'pt' ? 'Limpar Filtros e Ver Todos' : 'Clear Filters and View All'}
                    </button>
                  </div>
                </div>
              )}
              ...
              <section className="max-w-7xl mx-auto px-6">
                <div className="flex justify-between items-end mb-8 border-b border-gray-100 pb-4">
                  <div>
                    <span className="text-[11px] font-extrabold text-yellow-600 uppercase tracking-widest block mb-1">
                      {language === 'pt' ? 'Portfólio de Luxo' : 'Luxury Portfolio'}
                    </span>
                ...
                <div className="mb-8 flex flex-wrap items-center justify-between gap-4 p-4 rounded-xl border border-gray-100 bg-white shadow-sm">
                  <div className="flex items-center gap-2.5 text-left">
                    <span className={`w-2 h-2 rounded-full ${isConfigured ? 'bg-green-500 animate-pulse' : 'bg-yellow-500'}`} />
                    <div>
                      <p className="text-xs font-bold text-gray-950">
                        {isConfigured ? 'Banco de Dados Firestore Conectado' : 'Modo Demonstrativo Ativo (Pronto para Firebase)'}
                      </p>
                      <p className="text-[10px] text-gray-500 leading-tight">
                        {isConfigured 
                          ? 'Os imóveis exibidos abaixo estão sincronizados com o seu banco de dados Firebase.'
                          : 'O sistema de consulta dinâmica está ativo e pronto. As informações serão lidas do Firestore assim que configurado.'}
                      </p>
                    </div>
                  </div>
                  ...
                  {isConfigured && (
                    <button
                      ...
                      className="px-3.5 py-1.5 bg-gray-950 hover:bg-yellow-600 disabled:bg-gray-400 text-white text-[11px] font-bold rounded-lg transition-colors cursor-pointer flex items-center gap-1.5"
                    >
                      {syncStatus === 'syncing' && 'Sincronizando...'}
                      {syncStatus === 'success' && '✓ Sincronizado!'}
                      {syncStatus === 'error' && '✕ Erro ao Sincronizar'}
                      {syncStatus === 'idle' && 'Importar Imóveis de Exemplo'}
                    </button>
                  )}
                </div>
                      ...
                      >
                        ...
                        <div className="p-6 flex-grow flex flex-col justify-between space-y-4">
                          ...
                          <div className="pt-2 border-t border-gray-800 flex justify-between items-center text-xs text-gray-400">
                            <span>{broker.yearsOfExperience} {language === 'pt' ? 'Anos' : 'Years'} Exp</span>
              ...
              <section id="contato-secao" className="max-w-4xl mx-auto px-6 py-8">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                >
                  ...
                  {language === 'pt' ? 'Voltar para os imóveis' : 'Back to properties'}
                </button>
                <div className="text-[11px] font-sans text-gray-400 uppercase tracking-widest">
                  Home &gt; {selectedProperty.location.split(',')[0]} &gt;{' '}
                  <span className="text-gray-900 font-bold">{selectedProperty.title}</span>
                </div>
              </div>
              ...
              <div className="flex flex-col lg:flex-row justify-between items-start gap-6 border-b border-gray-100 pb-6">
                <div>
                  <div className="flex gap-2 mb-2">
                    ...
                    {selectedProperty.status && selectedProperty.status !== 'available' && (
                      <span className={`text-[9px] font-extrabold uppercase px-2 py-0.5 rounded-full tracking-wider font-bold ${
                        selectedProperty.status === 'interested'
                          ? 'bg-amber-500 text-black'
                          : 'bg-red-600 text-white'
                      }`}>
                        {selectedProperty.status === 'interested' ? '🟡 Reservado' : '🔴 Vendido'}
                      </span>
                    )}
                    {(!selectedProperty.status || selectedProperty.status === 'available') && selectedProperty.badge && (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 relative rounded-2xl overflow-hidden bg-gray-50 h-[450px]">
                  <img
                    src={selectedProperty.images[activeImageIndexes[selectedProperty.id] || 0]}
                  ...
                  {selectedProperty.images.length > 1 && (
                    <>
                      <button
                        onClick={() => {
                          const currentIdx = activeImageIndexes[selectedProperty.id] || 0;
                          const nextIdx = (currentIdx - 1 + selectedProperty.images.length) % selectedProperty.images.length;
                          setActiveImageIndexes(prev => ({ ...prev, [selectedProperty.id]: nextIdx }));
                        }}
                      ...
                      <button
                        onClick={() => {
                          const currentIdx = activeImageIndexes[selectedProperty.id] || 0;
                          const nextIdx = (currentIdx + 1) % selectedProperty.images.length;
                          setActiveImageIndexes(prev => ({ ...prev, [selectedProperty.id]: nextIdx }));
                        }}
                  ...
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 bg-black/40 backdrop-blur-md py-1.5 px-3 rounded-full">
                    {selectedProperty.images.map((_, idx) => (
                      <button
                        key={idx}
                        onClick={() => setActiveImageIndexes(prev => ({ ...prev, [selectedProperty.id]: idx }))}
                        className={`w-2 h-2 rounded-full transition-all ${
                          (activeImageIndexes[selectedProperty.id] || 0) === idx ? 'bg-white scale-125' : 'bg-white/40'
                        }`}
                      />
                    ))}
                  </div>
                ...
                <div className="flex flex-col gap-6 justify-between h-full">
                  ...
                  {selectedProperty.images.length > 1 ? (
                    <div className="grid grid-cols-2 gap-4">
                      {selectedProperty.images.map((img, index) => (
                        <div
                          key={index}
                          onClick={() => setActiveImageIndexes(prev => ({ ...prev, [selectedProperty.id]: index }))}
                          className={`h-24 rounded-xl overflow-hidden cursor-pointer border-2 transition-all relative ${
                            (activeImageIndexes[selectedProperty.id] || 0) === index ? 'border-yellow-600 scale-98 shadow-md' : 'border-transparent opacity-80 hover:opacity-100'
                          }`}
                        >
                    ...
                    <div className="bg-yellow-600/5 p-4 rounded-xl border border-yellow-600/10">
                      <p className="text-[10px] text-yellow-700 font-extrabold uppercase tracking-widest mb-1">{language === 'pt' ? 'Atendimento Premium' : 'Exclusive Treatment'}</p>
                  ...
                  {(propertyOwner || propertyBuyers.length > 0 || (selectedProperty.status && selectedProperty.status !== 'available')) && (
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-md space-y-4 text-left">
                      <h3 className="font-sans text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1.5 border-b border-gray-50 pb-2">
                        <span className="text-sm">📋</span> {language === 'pt' ? 'Ficha de Acompanhamento' : 'Tracking Sheet'}
                      </h3>

                      {propertyOwner && (
                        <div className="space-y-1">
                          <span className="text-[10px] text-gray-400 uppercase tracking-wider font-extrabold block">
                            {language === 'pt' ? 'Proprietário / Vendedor' : 'Owner / Seller'}
                          </span>
                          <p className="text-xs font-bold text-gray-950">{propertyOwner.name}</p>
                          <p className="text-[10px] text-gray-500 font-mono">{propertyOwner.phone} • {propertyOwner.email}</p>
                        </div>
                      )}
                      ...
                      {propertyBuyers.length > 0 && (
                        <div className="space-y-2 pt-2 border-t border-gray-100">
                          <span className="text-[10px] text-gray-400 uppercase tracking-wider font-extrabold block">
                            {language === 'pt' ? 'Clientes Interessados' : 'Interested Clients'}
                          </span>
                          <div className="space-y-2">
                            {propertyBuyers.map(buyer => (
                              <div key={buyer.id} className="bg-gray-50 p-3 rounded-xl border border-gray-100 flex justify-between items-center gap-3">
                                <div className="space-y-0.5 min-w-0 flex-1">
                                  <p className="text-xs font-bold text-gray-900 truncate">{buyer.name}</p>
                                  <div className="flex items-center gap-2 text-[10px]">
                                    <span className="text-gray-500 font-mono">{buyer.phone}</span>
                                    <span className={`px-1.5 py-0.5 rounded font-bold text-[8px] uppercase tracking-wider ${
                                      ...
                                      {buyer.buyerStatus === 'signed_contract' ? (language === 'pt' ? 'Vendido' : 'Sold') : (language === 'pt' ? 'Interessado' : 'Interested')}
                                    </span>
                                  </div>
                                </div>
                                <button
                                  onClick={() => {
                                    setSelectedContractProperty(selectedProperty);
                                    setSelectedContractBuyer(buyer);
                                    setSelectedContractOwner(propertyOwner || undefined);
                                    setContractModalOpen(true);
                                  }}
                                  title={language === 'pt' ? 'Gerar Contrato de Venda' : 'Generate Contract'}
                      ...
                      {selectedProperty.status && selectedProperty.status !== 'available' && (
                        <div className="pt-2 border-t border-gray-100">
                          <span className="text-[10px] text-gray-400 uppercase tracking-wider font-extrabold block mb-1">
                            {language === 'pt' ? 'Status de Negociação' : 'Deal Status'}
                          </span>
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider inline-block ${
                            ...
                            {selectedProperty.status === 'sold' 
                              ...
                              : (language === 'pt' ? '🟡 Reservado' : '🟡 Reserved')
                            }
                          </span>
                        </div>
                      )}
                  ...
                  <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-md space-y-4">
                    <h3 className="font-sans text-xs font-bold text-gray-400 uppercase tracking-wider">
                      {language === 'pt' ? 'Detalhamento Mensal' : 'Monthly Breakdown'}
                    </h3>
                    
                    <div className="space-y-2 text-sm">
                      {selectedProperty.condominio && selectedProperty.condominio > 0 ? (
                        <div className="flex justify-between py-1 border-b border-gray-50">
                          <span className="text-gray-500">{dict.condominio}</span>
                          <span className="font-bold text-gray-900">{formatBRL(selectedProperty.condominio)}</span>
                        </div>
                      ) : null}
                      {selectedProperty.iptu && selectedProperty.iptu > 0 ? (
                        <div className="flex justify-between py-1 border-b border-gray-50">
                          <span className="text-gray-500">{dict.iptu} ({language === 'pt' ? 'ano' : 'year'})</span>
                          <span className="font-bold text-gray-900">{formatBRL(selectedProperty.iptu)}</span>
                        </div>
                      ) : null}
                      <div className="flex justify-between py-1">
                        <span className="text-gray-500">{dict.area}</span>
                        <span className="font-bold text-gray-900">{selectedProperty.area} m²</span>
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="lg:col-span-2 space-y-10">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-white p-6 rounded-xl border border-gray-100 shadow-sm text-center">
                    <div className="space-y-1">
                      <span className="text-[10px] text-gray-400 uppercase tracking-wider block font-bold">{dict.suites}</span>
                    ...
                    <div className="space-y-1 border-t sm:border-t-0 sm:border-l border-gray-100 pt-3 sm:pt-0">
                      <span className="text-[10px] text-gray-400 uppercase tracking-wider block font-bold">{dict.area}</span>
                      <span className="text-xl font-serif font-bold text-gray-900 flex items-center justify-center gap-1.5">
                    ...
                    <div className="space-y-1 border-t sm:border-t-0 sm:border-l border-gray-100 pt-3 sm:pt-0">
                      <span className="text-[10px] text-gray-400 uppercase tracking-wider block font-bold">{dict.vagas}</span>
                      <span className="text-xl font-serif font-bold text-gray-900 flex items-center justify-center gap-1.5">
                    ...
                    <div className="space-y-1 border-t sm:border-t-0 sm:border-l border-gray-100 pt-3 sm:pt-0">
                      <span className="text-[10px] text-gray-400 uppercase tracking-wider block font-bold">{language === 'pt' ? 'Banheiros' : 'Bathrooms'}</span>
                      <span className="text-xl font-serif font-bold text-gray-900 flex items-center justify-center gap-1.5">
                        <span className="text-lg">🚿</span>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {selectedProperty.comodidades.map((item) => (
                        <div key={item} className="flex items-center gap-2 bg-white px-4 py-3 rounded-lg border border-gray-50 text-xs text-gray-700 font-medium">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                      <div className="space-y-4 text-xs md:text-sm text-gray-600">
                        ...
                        {selectedProperty.showAddressOnSite && selectedProperty.address && (
                          <div className="bg-yellow-50/50 border border-yellow-100 rounded-xl p-3 flex gap-2 items-start text-xs text-gray-800">
                            ...
                            <div>
                              <span className="font-bold text-gray-900 block">{language === 'pt' ? 'Endereço:' : 'Address:'}</span>
                        ...
                        <div className="space-y-3">
                          {selectedProperty.closestPlaces && selectedProperty.closestPlaces.map((place) => (
                            <div key={place.name} className="flex justify-between border-b border-gray-100 pb-2">
                              <span className="font-medium text-gray-800">{place.name}</span>
                              <span className="text-yellow-600 font-bold">{place.time}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  <div className="bg-gray-950 text-white rounded-2xl p-6 shadow-xl border border-gray-800 space-y-6">
                    ...
                    <div className="space-y-3.5 text-xs text-gray-300">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <span>{dict.creci}: {propertyBroker.creci}</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <button
                        onClick={() => {
                          const contactFormElement = document.getElementById('agendar-visita-form');
              ...
              <section className="pt-10 border-t border-gray-100">
                <div className="mb-8">
                  <span className="text-[11px] font-extrabold text-yellow-600 uppercase tracking-widest block mb-1">
                    {language === 'pt' ? 'Oportunidades Relacionadas' : 'Curated Match'}
                  </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {properties.filter((p) => p.id !== selectedProperty.id).slice(0, 3).map((property) => (
                    <PropertyCard
                      key={property.id}
                      property={property}
                      isFavorite={favorites.includes(property.id)}
          ...
          {currentView === 'broker-detail' && (
            ...
            >
              ...
              >
                <ChevronLeft className="w-4 h-4" />
                {language === 'pt' ? 'Voltar para os especialistas' : 'Back to specialists'}
              </button>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="lg:col-span-2 space-y-10">
                  ...
                  <div className="bg-gray-950 text-white p-8 rounded-2xl relative overflow-hidden shadow-xl border border-gray-800">
                    <div className="absolute top-4 right-6 text-6xl text-yellow-600/25 font-serif italic font-black">“</div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      {properties.filter((p) => p.brokerId === selectedBroker.id).map((property) => (
                        <PropertyCard
                          key={property.id}
                          property={property}
                          isFavorite={favorites.includes(property.id)}
                  ...
                  <div className="bg-white p-8 rounded-xl border border-gray-100 shadow-md">
                    <h3 className="font-serif text-xl font-bold text-gray-900 mb-4">
                      {language === 'pt' ? `Enviar uma Mensagem Direta para ${selectedBroker.name}` : `Send a Direct Message to ${selectedBroker.name}`}
                    </h3>
                    <ContactForm
                      language={language}
          ...
          {currentView === 'about' && (
            ...
            >
              ...
              <div className="space-y-4">
                <div className="text-[11px] font-sans text-gray-400 uppercase tracking-widest">
                  Home &gt; <span className="text-gray-900 font-bold">{language === 'pt' ? 'Sobre Nós' : 'About Us'}</span>
                </div>
                <div className="space-y-2">
                  <span className="text-[11px] font-extrabold text-yellow-600 uppercase tracking-widest block">
                    {language === 'pt' ? aboutUsConfig.heroSubtitlePt : aboutUsConfig.heroSubtitleEn}
                  </span>
                  <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl font-black text-gray-950 tracking-tight leading-tight">
                    {language === 'pt' ? aboutUsConfig.heroTitlePt : aboutUsConfig.heroTitleEn}
                  </h1>
                  <p className="text-gray-500 text-sm md:text-base max-w-3xl leading-relaxed">
                    {language === 'pt' ? aboutUsConfig.heroDescriptionPt : aboutUsConfig.heroDescriptionEn}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                ...
                <div className="lg:col-span-5 space-y-8">
                  <div className="relative aspect-[4/3] rounded-2xl overflow-hidden shadow-2xl border border-gray-100 bg-gray-900 group">
                    <img
                      src={aboutUsConfig.officeImage || "https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&q=80"}
                      alt="Office"
                      referrerPolicy="no-referrer"
                      loading="lazy"
                      decoding="async"
                    ...
                    <div className="absolute bottom-6 left-6 right-6">
                      <span className="text-[10px] uppercase font-bold text-yellow-500 tracking-wider">
                        {language === 'pt' ? aboutUsConfig.officeImageBadgePt : aboutUsConfig.officeImageBadgeEn}
                      </span>
                      <h4 className="font-serif text-lg font-bold text-white mt-1">{aboutUsConfig.officeAddress}</h4>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm text-center">
                    <div className="space-y-1">
                      <span className="block font-serif text-3xl font-black text-gray-950">{aboutUsConfig.stat1Value}</span>
                      <span className="text-[10px] text-gray-500 uppercase tracking-widest font-bold block">
                        {language === 'pt' ? aboutUsConfig.stat1LabelPt : aboutUsConfig.stat1LabelEn}
                      </span>
                    </div>
                    <div className="space-y-1 border-l border-gray-100">
                      <span className="block font-serif text-3xl font-black text-gray-950">{aboutUsConfig.stat2Value}</span>
                      <span className="text-[10px] text-gray-500 uppercase tracking-widest font-bold block">
                        {language === 'pt' ? aboutUsConfig.stat2LabelPt : aboutUsConfig.stat2LabelEn}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:col-span-7 space-y-6">
                  <div className="space-y-4">
                    <h3 className="font-serif text-2xl font-bold text-gray-900">
                      {language === 'pt' ? aboutUsConfig.contentTitlePt : aboutUsConfig.contentTitleEn}
                    </h3>
                    {(language === 'pt' ? aboutUsConfig.contentParagraphsPt : aboutUsConfig.contentParagraphsEn)
                      .split('\n')
                      .filter(p => p.trim())
                      .map((p, idx) => (
                        <p key={idx} className="text-gray-600 text-sm leading-relaxed font-sans">
                          {p}
                        </p>
                      ))}
                  </div>
                  ...
                  <div className="space-y-4 pt-6 border-t border-gray-100">
                    <h4 className="font-sans text-xs font-bold text-yellow-600 uppercase tracking-widest">
                      {language === 'pt' ? 'Nossos Pilares Fundamentais' : 'Our Fundamental Pillars'}
                    </h4>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                      <div className="space-y-1">
                        <span className="text-yellow-600 font-bold text-xs block">✧ {language === 'pt' ? aboutUsConfig.pillar1TitlePt : aboutUsConfig.pillar1TitleEn}</span>
                        <p className="text-gray-500 text-[11px] leading-normal">
                          {language === 'pt' ? aboutUsConfig.pillar1DescPt : aboutUsConfig.pillar1DescEn}
                        </p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-yellow-600 font-bold text-xs block">✧ {language === 'pt' ? aboutUsConfig.pillar2TitlePt : aboutUsConfig.pillar2TitleEn}</span>
                        <p className="text-gray-500 text-[11px] leading-normal">
                          {language === 'pt' ? aboutUsConfig.pillar2DescPt : aboutUsConfig.pillar2DescEn}
                        </p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-yellow-600 font-bold text-xs block">✧ {language === 'pt' ? aboutUsConfig.pillar3TitlePt : aboutUsConfig.pillar3TitleEn}</span>
                        <p className="text-gray-500 text-[11px] leading-normal">
                          {language === 'pt' ? aboutUsConfig.pillar3DescPt : aboutUsConfig.pillar3DescEn}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="bg-gray-50 rounded-2xl border border-gray-100 overflow-hidden shadow-sm grid grid-cols-1 lg:grid-cols-12">
                <div className="lg:col-span-5 p-8 md:p-12 space-y-6 flex flex-col justify-center">
                  <div>
                    <span className="text-[10px] font-bold text-yellow-600 uppercase tracking-widest block mb-1">Visite-nos</span>
                    <h3 className="font-serif text-2xl font-bold text-gray-950">Nossa Sede</h3>
                  </div>
                  <p className="text-gray-600 text-xs md:text-sm leading-relaxed">
                    Estamos à sua inteira disposição no coração de Caxias. Venha desfrutar de um café conosco e planejar seu próximo passo patrimonial com nossos especialistas seniores.
                  </p>
                  <div className="space-y-2 text-xs text-gray-600 font-sans">
                    <p className="font-bold text-gray-900">FA Imóveis Luxury Estate</p>
                    <p>Rua São Pedro, 263 - Centro</p>
                    <p>Caxias - MA, CEP: 65600-000</p>
                    <p className="pt-2"><span className="font-bold text-gray-900">Horário:</span> Seg a Sex, das 8h às 18h</p>
                  </div>
                </div>
              ...
              <div className="max-w-4xl mx-auto space-y-8 pt-8">
                <div className="text-center space-y-2">
                  <span className="text-[11px] font-bold text-yellow-600 uppercase tracking-widest block">Fale Conosco</span>
                  <h2 className="font-serif text-3xl font-bold text-gray-900">Fale com a Diretoria</h2>
                  <p className="text-gray-500 text-xs md:text-sm max-w-md mx-auto">
                    Preencha o formulário abaixo para enviar uma mensagem oficial. Nosso tempo médio de resposta via WhatsApp é de poucos minutos.
                  </p>
                </div>
                <ContactForm
                  language={language}
          ...
          {currentView === 'favorites' && (
            ...
            >
              <div>
                <span className="text-[11px] font-extrabold text-yellow-600 uppercase tracking-widest block mb-1">
                  {language === 'pt' ? 'Minha Seleção' : 'My Curated Collection'}
                </span>
              ...
              {favorites.length === 0 ? (
                <div className="text-center py-24 bg-white rounded-2xl border border-gray-100 space-y-4">
                  ...
                  >
                    {language === 'pt' ? 'Ir para a página inicial' : 'Go back to home'}
                  </button>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              {checkingAuth ? (
                <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
                  ...
                  <p className="text-xs text-gray-400 font-mono tracking-widest uppercase">
                    {language === 'pt' ? 'Verificando credenciais...' : 'Checking auth status...'}
                  </p>
                </div>
              ) : adminUser ? (
                <AdminPanel
